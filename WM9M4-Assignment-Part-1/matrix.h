...
public:
    ...
    vec4 operator * (const vec4& v) const {
        ...
        //vec4 result;
        //__m128 vec = _mm_set_ps(v[3], v[2], v[1], v[0]);

        //for (int i = 0; i < 4; ++i) {
        //    __m128 row = _mm_loadu_ps(m[i]);
        //    __m128 result128 = _mm_dp_ps(row, vec, 0xF1); 
        //    result[i] = _mm_cvtss_f32(result128);
        //}
        //return result;
    ...
    matrix operator * (const matrix& mx) const {
        ...
        //        __m128 rowVec = _mm_loadu_ps(&a[row * 4]);
        //        __m128 colVec = _mm_set_ps(mx.a[3 * 4 + col], mx.a[2 * 4 + col], mx.a[1 * 4 + col], mx.a[0 * 4 + col]);
        //        __m128 sumResult = _mm_dp_ps(rowVec, colVec, 0xF1);
        //        ret.a[row * 4 + col] = _mm_cvtss_f32(sumResult);
        //    }
        //}
